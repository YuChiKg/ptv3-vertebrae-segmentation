...
"""
adding num_examples_per_specimen to find the minimun number of examples to train the model and reach the best performance 
as using all the examples
"""

class TrainDataset(Dataset):
    def __init__(self, root_dir, num_points=1024, test_specimen_idx=9, sample_ratio=0.2, num_examples=25, transforms=None):
        """
        Training dataset: Ensures a fixed number of examples per specimen.
        """
    ...
    def calculate_label_weights(self):
        """Calculate label weights to handle class imbalance before sampling."""
        total_labels = np.zeros(2)  # Two labels: 0 and 1
        for file_name in self.files:
            file_path = os.path.join(self.root_dir, file_name)
            specimen_data = np.load(file_path)
            labels = specimen_data[:, 9]
            labels_binary = np.where(labels > 0, 1, 0)  # Convert to binary labels

            # Count occurrences of each label
            tmp, _ = np.histogram(labels_binary, bins=[0, 1, 2])
            total_labels += tmp
    ...
    def __getitem__(self, idx):
        ...
        # Convert labels 1-5 to 1 for binary classification
        points[:, 9] = np.where(points[:, 9] > 0, 1, 0)

        # Separate class 0 and class 1 points
        class_0 = points[points[:, 9] == 0]
        class_1 = points[points[:, 9] == 1]

        # Set a target ratio
        num_class_1 = min(int(self.num_points * self.sample_ratio), len(class_1))
        num_class_0 = self.num_points - num_class_1

        class_0_sample = class_0[np.random.choice(len(class_0), num_class_0, replace=True)]
        class_1_sample = class_1[np.random.choice(len(class_1), num_class_1, replace=True)]

        sampled_points = np.concatenate([class_0_sample, class_1_sample], axis=0)
        ...
        # for training combined datasets with "Antonin" and "7D Phantom"
        features = sampled_points[:, :6]  # [x, y, z, r, g, b]
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        num_class_1 = min(int(self.num_points * 0.2), len(class_1))
        num_class_0 = self.num_points - num_class_1

        class_0_sample = class_0[np.random.choice(len(class_0), num_class_0, replace=False)]
        class_1_sample = class_1[np.random.choice(len(class_1), num_class_1, replace=False)]

        sampled_points = np.concatenate([class_0_sample, class_1_sample], axis=0)
        
######### Set rgb to set values
        # sampled_points[:, 3:6] = 255
